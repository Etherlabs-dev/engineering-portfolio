...
- Model evidence: Seedance 2.5, Seedance 2.0, Kling 3.0/Omni, Veo 3.1/Lite, Luma Ray3.2, Runway Gen-4.5 and Aleph 2.0 are verified catalog profiles with deterministic local adapters. The official ByteDance Seed page supplied on 2026-08-12 confirms Seedance 2.5 joint audio-video generation, up-to-30-second output, reference control, editing and professional production controls; it does not state a release date, so the catalog records the verification date. Seedance 2.5 is the primary deterministic multimodal-story route. Sora 2 remains disabled as unavailable. No provider call is claimed.
...
- Public proof: `main` is published at `https://github.com/Etherlabs-dev/multimodal-video-pipeline`; Seedance 2.5 verification commit `b7cafe5a9823ce1efdbef2d44e8a7fa7935d4b2e` passed every GitHub Actions gate in run `31594882617`. The README links the case study, architecture, security, evaluation, runbook, evidence, production lessons, approved references and final screenshots.
